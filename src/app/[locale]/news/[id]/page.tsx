...
function useNews(postId: string) {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            if (!postId) {
                setLoading(false);
                setError(new Error("Invalid new ID"));
                return;
            }
...
export default function NewsIdPage({
    ...
    const { news, loading, error } = useNews(pageId);
    ...
    if (error || !news) return <div>Error loading post</div>;
        ...
        <section className="flex min-h-screen items-center justify-center bg-white px-2 py-6">
            <div className="max-w-2xl space-y-6">
                ...
                <div className="space-y-2 px-2">
                    {news.titleDescriptions?.map((desc, index) => (
                ...
                <div className="space-y-6 text-2xl font-light">
                    {news.descriptions?.map((desc, index) => (
                        <p key={index}>{desc.value}</p>
                    ))}
                </div>
                {news.lists?.map((list, listIndex) => (
                    <div
                        key={listIndex}
